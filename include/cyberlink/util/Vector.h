...
template <typename T> class SharedVector : public std::vector< cyber_shared_ptr<T> > {
private:
 ...
 public:
  ...
  bool add(cyber_shared_ptr<T> obj) {
    if (!obj)
      return false;
    if (0 <= indexOf(obj))
      return false;
    std::vector< cyber_shared_ptr<T> >::push_back(obj);
    return true;
  }

  // insertAt
  
  bool insertAt(cyber_shared_ptr<T> obj, size_t index) {
    if (!obj)
      return false;
    if (0 <= indexOf(obj))
      return false;
    std::vector< cyber_shared_ptr<T> >::insert((std::vector< cyber_shared_ptr<T> >::begin() + index), obj);
    return true;
  }

  // get
  
  cyber_shared_ptr<T> get(size_t index) {
    if (std::vector< cyber_shared_ptr<T> >::size() < (index+1))
      return cyber_shared_ptr<T>((T*)NULL);
    return std::vector< cyber_shared_ptr<T> >::at(index);
  }

  // exists
  
  bool exists(cyber_shared_ptr<T> obj) {
  ...
  ssize_t indexOf(cyber_shared_ptr<T> obj) {
    if (!obj)
      return -1;
    size_t cnt = std::vector< cyber_shared_ptr<T> >::size();
    for (size_t n = 0; n < cnt; n++) {
	  cyber_shared_ptr<T> sobj = std::vector< cyber_shared_ptr<T> >::at(n);
	  if (obj.get() == sobj.get())
		  return n;
	}
    return -1;
  ...
  bool remove(size_t idx) {
    size_t size = std::vector< cyber_shared_ptr<T> >::size();
    if ((size <= 0) || (size < (idx+1)))
      return false;
    typename std::vector< cyber_shared_ptr<T> >::iterator objIt = std::vector< cyber_shared_ptr<T> >::begin() + idx;
    std::vector< cyber_shared_ptr<T> >::erase(objIt);
    return true;
  }
  
  bool remove(cyber_shared_ptr<T> obj) {
    if (!obj)
      return false;
