...
#include <device/InvalidDescriptionException.h>
#include <xml/DeviceData.h>
#include <event/SubscriptionRequest.h>
#include <event/SubscriptionResponse.h>
#include <device/Advertiser.h>
#include <util/Mutex.h>

#include <sstream>

namespace CyberLink {
class Device : public CyberLink::HTTPRequestListener, public SearchListener {
  cyber_shared_ptr<uXML::Node> rootNode;
  cyber_shared_ptr<uXML::Node> deviceNode;
 ...
 public:
  ...
  void setDeviceNode(cyber_shared_ptr<uXML::Node> node) {
    deviceNode = node;
  }
  ...
  Device();
  Device(cyber_shared_ptr<uXML::Node> root, cyber_shared_ptr<uXML::Node> device);
  Device(cyber_shared_ptr<uXML::Node> device);
#if !defined(BTRON) && !defined(ITRON) && !defined(TENGINE) 
  Device(CyberLink::File *descriptionFile);
...
public:
 ...
 public:
  ...
  bool isDescriptionURI(const std::string &uri) {
    ...
    if (!descriptionURI)
      return false;
    CyberLink::String descriptionURIStr(descriptionURI);
...
#if !defined(BTRON) && !defined(ITRON) && !defined(TENGINE) 
  bool loadDescription(CyberLink::File *file);
#endif
...
private:
 ...
 public:
  static bool isDeviceNode(cyber_shared_ptr<uXML::Node> node) {
...
private:
 ...
 public:
  ...
  cyber_shared_ptr<Device> getDeviceByDescriptionURI(const std::string &uri);
...
private:
  ...
  CyberLink::HTTP::StatusCode httpGetRequestRecieved(CyberLink::HTTPRequest *httpReq);
  CyberLink::HTTP::StatusCode httpPostRequestRecieved(CyberLink::HTTPRequest *httpReq);
...
private:
  ...
  CyberLink::HTTP::StatusCode soapBadActionRecieved(CyberLink::HTTPRequest *soapReq);
  CyberLink::HTTP::StatusCode soapActionRecieved(CyberLink::HTTPRequest *soapReq);
...
private:
  ...
  CyberLink::HTTP::StatusCode invalidActionControlRecieved(ControlRequest *ctlReq);
  CyberLink::HTTP::StatusCode deviceControlRequestRecieved(ControlRequest *ctlReq, Service *service);
  CyberLink::HTTP::StatusCode deviceActionControlRecieved(ActionRequest *ctlReq, Service *service);
  CyberLink::HTTP::StatusCode deviceQueryControlRecieved(QueryRequest *ctlReq, Service *service);
...
private:
  ...
  CyberLink::HTTP::StatusCode upnpBadSubscriptionRecieved(SubscriptionRequest *subReq, int code);
  CyberLink::HTTP::StatusCode deviceEventSubscriptionRecieved(SubscriptionRequest *subReq);
  CyberLink::HTTP::StatusCode deviceEventNewSubscriptionRecieved(Service *service, SubscriptionRequest *subReq);
  CyberLink::HTTP::StatusCode deviceEventRenewSubscriptionRecieved(Service *service, SubscriptionRequest *subReq);
  CyberLink::HTTP::StatusCode deviceEventUnsubscriptionRecieved(Service *service, SubscriptionRequest *subReq);
