...
#include <media/server/object/ContentPropertyList.h>
...
class ContentNode : public uXML::Node
{
...
public:
	...
	{
		return (ContentNode *)getNode(index).get();
	}

	ContentNode *getContentNode(const char *name) 
	{
		return (ContentNode *)getNode(name).get();
	}
	
	virtual void addContentNode(cyber_shared_ptr<ContentNode> node) = 0;
	virtual bool removeContentNode(cyber_shared_ptr<ContentNode> node) = 0;
