...
class HTTPPacket  {
  ...
  CyberLink::InputStream *contentInput;
...
public:
  ...
  HTTPPacket(CyberLink::InputStream *in);
...
public:
 ...
 public:
  ...
  size_t getLongLongHeaderValue(const std::string &name) {
    ...
    if (!header)
      return 0;
    return (size_t)atoll(header->getValue());
  }
  ...
  void setContentInputStream(CyberLink::InputStream *in) {
    contentInput = in;
  }

  CyberLink::InputStream *getContentInputStream() {
    return contentInput;
  }
