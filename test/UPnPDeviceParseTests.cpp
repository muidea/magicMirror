...
#include "UPnPDeviceParseTests.h"
...
void CyberLinkTest::TestDevice01()
{
  Device *dev = new Device();
  if (!dev->loadDescription(UPNP_TEST_DEVICE_DESCR_01)) {
	  std::cout << "dev->loadDescription failed, :" << __LINE__ << std::endl;
  }
  if (strcmp(dev->getDeviceType(), "urn:schemas-upnp-org:device:clock:1") != 0) {
	  std::cout << "dev->getDeviceType failed, :" << __LINE__ << std::endl;
  }
  delete dev;
}
...
void CyberLinkTest::TestDevice02()
{
  Device *dev = new Device();
  if (!dev->loadDescription(UPNP_TEST_DEVICE_DESCR_02)) {
	  std::cout << "dev->loadDescription failed, :" << __LINE__ << std::endl;
  }
  if (strcmp(dev->getDeviceType(), "urn:schemas-upnp-org:device:clock:1") != 0) {
	  std::cout << "dev->getDeviceType failed, :" << __LINE__ << std::endl;
