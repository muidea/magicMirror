...
Service::Service(cyber_shared_ptr<uXML::Node> node) {
  serviceNode = node;
...
void Service::initActionList() {
  actionList.clear();
  cyber_shared_ptr<uXML::Node> scdpNode = getSCPDNode();
  if (!scdpNode)
    return;
  cyber_shared_ptr<uXML::Node> actionListNode = scdpNode->getNode(ActionList::ELEM_NAME);
  if (!actionListNode)
    return;
  cyber_shared_ptr<uXML::Node> serviceNode = getServiceNode();
  size_t nNode = actionListNode->getNNodes();
  for (size_t n = 0; n < nNode; n++) {
    cyber_shared_ptr<uXML::Node> node = actionListNode->getNode(n);
...
void Service::initServiceStateTable() {
  ...
  cyber_shared_ptr<uXML::Node> scpdNode = getSCPDNode();
  if (!scpdNode)
    return;
  cyber_shared_ptr<uXML::Node> stateTableNode = scpdNode->getNode(ServiceStateTable::ELEM_NAME);
  if (!stateTableNode)
    return;
  cyber_shared_ptr<uXML::Node> serviceNode = getServiceNode();
  size_t nNode = stateTableNode->getNNodes();
  for (size_t n = 0; n < nNode; n++) {
    cyber_shared_ptr<uXML::Node> node = stateTableNode->getNode(n);
...
cyber_shared_ptr<uXML::Node> Service::getSCPDNode(CyberLink::URL *url) {
  uXML::Parser parser;
...
cyber_shared_ptr<uXML::Node> Service::getSCPDNode(CyberLink::File *file) {
  uXML::Parser parser;
...
cyber_shared_ptr<uXML::Node> Service::getSCPDNode(const std::string &description) {
  uXML::Parser parser;
  return parser.parse(description);
}

cyber_shared_ptr<uXML::Node> Service::getSCPDNode() {
  ServiceData *data = getServiceData();
  cyber_shared_ptr<uXML::Node> scpdNode = data->getSCPDNode();
  if (scpdNode)
    return scpdNode;
...
bool Service::loadSCPD(CyberLink::File *file) {
  string description;
...
bool Service::notify(Subscriber *sub, StateVariable *stateVar) {
  ...
  CyberLink::HTTPResponse *res = notifyReq.post(host, port);
