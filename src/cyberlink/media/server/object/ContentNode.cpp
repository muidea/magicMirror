...
void ContentNode::set(uXML::Node *node)
{
	int n;
	...
	for (n=0; n<nNode; n++) {
		cyber_shared_ptr<Node> cnode = node->getNode(n);
		if (ContainerNode::isContainerNode(cnode.get()) == true)
			continue;
		if (ItemNode::isItemNode(cnode.get()) == true)
			continue;
	...
	for (n=0; n<nAttr; n++) {
		uXML::Attribute *attr = node->getAttribute(n);
...
void ContentNode::outputPropertyAttributes(std::ostream& ps, ContentProperty *prop)
{
	...
	for (int n=0; n<nAttributes; n++) {
		uXML::Attribute *attr = prop->getAttribute(n);
...
void ContentNode::output(std::ostream& ps, int indentLevel, bool hasChildNode) 
{
	...
	if (hasChildNode == true) {
		...
		for (int n=0; n<nChildNodes; n++) {
			cyber_shared_ptr<Node> cnode = getNode(n);
