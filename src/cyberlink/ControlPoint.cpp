...
cyber_shared_ptr<Device> ControlPoint::getDevice(cyber_shared_ptr<uXML::Node> rootNode) {
  if (!rootNode)
    return cyber_shared_ptr<Device>((Device *)nullptr);
  cyber_shared_ptr<uXML::Node> devNode = rootNode->getNode(Device::ELEM_NAME);
  if (!devNode)
    return cyber_shared_ptr<Device>((Device *)nullptr);
  return cyber_shared_ptr<Device>(new Device(rootNode, devNode));
}
...
void ControlPoint::initDeviceList() {
  ...
  for (size_t n = 0; n < nRoots; n++) {
    cyber_shared_ptr<uXML::Node> rootNode = devNodeList.getNode(n);
    cyber_shared_ptr<Device> dev = getDevice(rootNode);
    if (!dev)
      continue;
    deviceList.add(dev);
  }
...
cyber_shared_ptr<Device> ControlPoint::getDevice(const std::string &name) {
...
bool ControlPoint::addDevice(cyber_shared_ptr<uXML::Node> rootNode) {
...
bool ControlPoint::addDevice(SSDPPacket *ssdpPacket) {
  ...
  cyber_shared_ptr<Device> dev = getDevice(udn);
  if (dev) {
  ...
  uXML::Parser parser;
  cyber_shared_ptr<uXML::Node> rootNode = parser.parse(&locationURL);
  cyber_shared_ptr<Device> rootDev = getDevice(rootNode);
  if (!rootDev) {
    unlock();
    return false;
  }
...
bool ControlPoint::removeDevice(cyber_shared_ptr<uXML::Node> rootNode) {
...
void ControlPoint::removeExpiredDevices() {
  ...
  cyber_shared_ptr<Device> *dev = new cyber_shared_ptr<Device>[devCnt];
  ...
  for (n = 0; n < devCnt; n++) {
    ...
    if (dev[n]->isExpired() == true) {
      ...
      cyber_shared_ptr<uXML::Node> rootNode = dev[n]->getRootNode();
...
CyberLink::HTTP::StatusCode ControlPoint::httpRequestRecieved(HTTPRequest *httpReq) {
  if (Debug::isOn() == true)
    httpReq->print();
...
void ControlPoint::unsubscribe(Device *device) {
  ...
  for (n = 0; n < childDevCnt; n++) {
    cyber_shared_ptr<Device> cdev = childDevList->getDevice(n);
...
void ControlPoint::renewSubscriberService(Device *dev, long timeout) {
  ...
  for (n = 0; n < cdevCnt; n++) {
    cyber_shared_ptr<Device> cdev = cdevList->getDevice(n);
