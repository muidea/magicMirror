...
Action::Action(cyber_shared_ptr<uXML::Node> serviceNode, cyber_shared_ptr<uXML::Node> actionNode) {
...
void Action::initArgumentList() {
  ...
  cyber_shared_ptr<uXML::Node> argumentListNode = getActionNode()->getNode(ArgumentList::ELEM_NAME);
  if (!serviceNode || !argumentListNode)
    return;

  size_t nodeCnt = argumentListNode->getNNodes();
  for (size_t n = 0; n < nodeCnt; n++) {
    cyber_shared_ptr<uXML::Node> argNode = argumentListNode->getNode(n);
