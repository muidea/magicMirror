...
cyber_shared_ptr<uXML::Node> uSOAP::SOAP::CreateEnvelopeBodyNode() {
  // <Envelope>
  ...
  cyber_shared_ptr<uXML::Node> bodyNode(new uXML::Node(bodyNodeName.c_str()));
  envNode->addNode(bodyNode);

  return cyber_shared_ptr<uXML::Node>(envNode);
}
