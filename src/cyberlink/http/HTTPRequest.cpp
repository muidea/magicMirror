...
HTTPRequest::HTTPRequest() {
  ...
  defaultUserAgent << CyberLink::PRODUCT_NAME << "/" << CyberLink::LIBRARY_VERSION << " " << CyberLink::LIBRARY_NAME << "/" << CyberLink::LIBRARY_VERSION;
  setUserAgent(defaultUserAgent.str());
  
  setAccept("*/*");
}

HTTPRequest::HTTPRequest(cyber_shared_ptr<HTTPSocket> httpSock) : HTTPPacket(httpSock.get()) {
  setSocket(httpSock);
  postSocket = NULL;
  requestPort = -1;
}
...
void CyberLink::HTTPRequest::setURI(const std::string &value) {
  uri = value;
}

const char *CyberLink::HTTPRequest::getURI(std::string &uriBuf) {
  if (0 < uri.length())
    uriBuf = uri;
  else
...
void CyberLink::HTTPRequest::getURI(URI &uri) {
...
void CyberLink::HTTPRequest::setURL(const std::string &urlString) {
  URL url(urlString);
  setURL(&url);
}

void CyberLink::HTTPRequest::setURL(URL *url) {
...
ParameterList *HTTPRequest::getParameterList(ParameterList &paramList) {
  ...
  while (paramIdx != string::npos) {
	...
	cyber_shared_ptr<Parameter> param(new Parameter(name.c_str(), value.c_str()));
    paramList.add(param);
...
HTTP::StatusCode HTTPRequest::post(HTTPResponse *httpRes, bool isOnlyHeader) {
  cyber_shared_ptr<HTTPSocket> httpSock = getSocket();
  size_t offset = 0;
  ...
  if (hasContentRange() == true) {
    ...
    if (lastPos <= 0) 
      lastPos = length - 1;
    if (((size_t)firstPos > length ) || ((size_t)lastPos > length))
