...
void String::setValue(const std::string &value, int offset, int count)  {
  ...
  if ((int)value.length() < (offset + count)) 
    return;
...
int String::regionMatchesIgnoreCase(int toffset, const std::string &other, int ooffset, int len) {
  ...
  int value2Len = (int)strlen(other.c_str());
...
std::ostream& CyberLink::operator<<(std::ostream &s, String &value) {
...
bool CyberLink::StringHasData(const std::string &value) {
  if (0 < strlen(value.c_str()))
    return true;
  return false;
}

int CyberLink::StringLength(const std::string &value) {
  return (int)strlen(value.c_str());
}

int CyberLink::String2Integer(const std::string &value) {
  return atoi(value.c_str());
}

int CyberLink::HexString2Integer(const std::string &value) {
  return (int)strtol(value.c_str(), NULL, 16);
}

long CyberLink::String2Long(const std::string &value) {
  return atol(value.c_str());
}

long CyberLink::HexString2Long(const std::string &value) {
  return strtol(value.c_str(), NULL, 16);
}

size_t CyberLink::String2Sizet(const std::string &value) {
  return atol(value.c_str());
}

size_t CyberLink::HexString2Sizet(const std::string &value) {
  return strtol(value.c_str(), NULL, 16);
}

const char *CyberLink::Integer2String(int value, std::string &valueBuf) {
  // INT_MAX 2147483647
...
const char *CyberLink::Long2String(long value, std::string &valueBuf) {
...
const char *CyberLink::Integer2HexString(int value, std::string &valueBuf) {
  // INT_MAX 2147483647
   // UINT_MAX 4294967295U
  char strBuf[String::INT_STRING_MAXSIZE];
#if defined(HAVE_SNPRINTF)
...
const char *CyberLink::Long2HexString(long value, std::string &valueBuf) {
  // UINT_LONG : 18446744073709551615UL
  // LONG_MAX :  9223372036854775807L
  char strBuf[String::LONG_STRING_MAXSIZE];
#if defined(HAVE_SNPRINTF)
...
const char *CyberLink::Sizet2String(size_t value, std::string &valueBuf) {
...
const char *CyberLink::Sizet2HexString(size_t value, std::string &valueBuf) {
  std::stringstream strBuf;
  strBuf << hex << value;
  valueBuf = strBuf.str();
  return valueBuf.c_str();
...
bool CyberLink::StringEquals(const std::string &value1, const std::string &value2) {
  if (strcmp(value1.c_str(), value2.c_str()) == 0)
    return true;
  return false;
}

const char *CyberLink::StringToUpperCase(std::string &str) {
...
const char *CyberLink::StringToLowerCase(std::string &str) {
  int len = (int)str.length();
  for (int n = 0; n < len; n++)
    str[n] = tolower(str.at(n));
  return str.c_str();
}
bool CyberLink::StringEqualsIgnoreCase(const std::string &value1, const std::string &value2) {
...
bool CyberLink::StringStartsWith(const std::string &value, const std::string &startsWith) {
...
bool CyberLink::StringStartsWith(const std::string &value, const char startsWith) {
...
bool CyberLink::StringEndsWith(const std::string &value, const std::string &endsWith) {
...
bool CyberLink::StringEndsWith(const std::string &value, const char endsWith) {
  int len = StringLength(value);
  if (len <= 0)
    return false;
...
int CyberLink::StringIndexOf(const std::string &value, char c) {
  int len = StringLength(value);
  for (int n = 0; n < len; n++) {
    if (value[n] == c)
      return n;
...
const char *CyberLink::StringTrim(const std::string &value, const std::string &trimChars, std::string &buf) {
...
const char *CyberLink::StringTrim(const std::string &value, std::string &buf) {
  return StringTrim(value, " ", buf);
}

const char *CyberLink::StringReplaceChars(std::string &str, const std::string &fromChars, const std::string &toChanrs) {
