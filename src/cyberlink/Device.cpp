...
#include <ssdp/SSDPSearchResponse.h>
...
Device::Device(cyber_shared_ptr<uXML::Node> root, cyber_shared_ptr<uXML::Node> device) {
...
Device::Device() {
  setLocalRootDeviceFlag(false);
  rootNode = cyber_shared_ptr<uXML::Node>((uXML::Node *)nullptr);
  deviceNode = cyber_shared_ptr<uXML::Node>((uXML::Node *)nullptr);
  initUUID();
  updateBootID();
  initDeviceData();
  initChildList();
...
Device::Device(cyber_shared_ptr<uXML::Node> device) {
  setLocalRootDeviceFlag(false);
  rootNode = cyber_shared_ptr<uXML::Node>((uXML::Node *)nullptr);
  deviceNode = device;
  initUUID();
...
Device::Device(CyberLink::File *descriptionFile) {
...
Device::Device(const std::string &descriptionFileName) {
  ...
  initUUID();
  updateBootID();
  CyberLink::File descriptionFile(descriptionFileName);
  bool ret = loadDescription(&descriptionFile);
  if (ret == false)
    throw InvalidDescriptionException(INVALIDDESCRIPTIONEXCEPTION_FILENOTFOUND);
}
...
void Device::setNMPRMode(bool flag) {
  cyber_shared_ptr<uXML::Node> devNode = getDeviceNode();
  if (!devNode)
    return;
  if (flag == true) {
...
void Device::setURLBase(const std::string &value) {
  if (isRootDevice() == true) {
    cyber_shared_ptr<uXML::Node> node = getRootNode()->getNode(URLBASE_NAME);
    if (node) {
      node->setValue(value);
      return;
    }

	cyber_shared_ptr<uXML::Node> newNode(new uXML::Node(URLBASE_NAME));
...
Device *Device::getRootDevice() {
  if (rootDevice)
    return rootDevice;
  cyber_shared_ptr<uXML::Node> rootNode = getRootNode();
  if (!rootNode)
    return nullptr;
  cyber_shared_ptr<uXML::Node> devNode = rootNode->getNode(Device::ELEM_NAME);
  if (!devNode)
    return nullptr;
...
Device *Device::getParentDevice() {
  ...
  cyber_shared_ptr<uXML::Node> parentNode = devNode->getParentNode();
  if (!parentNode)
    return nullptr;
...
bool Device::loadDescription(CyberLink::File *file) {
  string description;
...
void Device::initDeviceList() {
  ...
  if (!devNode)
    return;
  cyber_shared_ptr<uXML::Node> devListNode = devNode->getNode(DeviceList::ELEM_NAME);
  if (!devListNode)
    return;
  size_t nNode = devListNode->getNNodes();
  for (size_t n = 0; n < nNode; n++) {
    cyber_shared_ptr<uXML::Node> node = devListNode->getNode(n);
    if (Device::isDeviceNode(node) == false)
      continue;

	cyber_shared_ptr<Device> dev(new Device(node));
    deviceList.add(dev);
  } 
}
...
cyber_shared_ptr<Device> Device::getDevice(const std::string &name) {
  DeviceList *devList = getDeviceList();
  size_t devCnt = devList->size();
  for (size_t n = 0; n < devCnt; n++) {
    cyber_shared_ptr<Device> dev = devList->getDevice(n);
    if (dev->isDevice(name) == true)
      return dev;
    cyber_shared_ptr<Device> cdev = dev->getDevice(name);
    if (cdev)
      return cdev;
  }
  return cyber_shared_ptr<Device>((Device *)nullptr);
}

cyber_shared_ptr<Device> Device::getDeviceByDescriptionURI(const std::string &uri) {
  DeviceList *devList = getDeviceList();
  size_t devCnt = devList->size();
  for (size_t n = 0; n < devCnt; n++) {
    cyber_shared_ptr<Device> dev = devList->getDevice(n);
    if (dev->isDescriptionURI(uri) == true)
      return dev;
    cyber_shared_ptr<Device> cdev = dev->getDeviceByDescriptionURI(uri);
    if (cdev)
      return cdev;
  }
  return cyber_shared_ptr<Device>((Device *)nullptr);
...
void Device::initServiceList() {
  ...
  if (!devNode)
    return;
  cyber_shared_ptr<uXML::Node> serviceListNode = devNode->getNode(ServiceList::ELEM_NAME);
  if (!serviceListNode)
    return;
  size_t nNode = serviceListNode->getNNodes();
  for (size_t n = 0; n < nNode; n++) {
    cyber_shared_ptr<uXML::Node> node = serviceListNode->getNode(n);
...
void Device::initIconList() {
  ...
  if (!devNode)
    return;
  cyber_shared_ptr<uXML::Node> iconListNode = devNode->getNode(IconList::ELEM_NAME);
  if (!iconListNode)
    return;
  size_t nNode = iconListNode->getNNodes();
  for (size_t n = 0; n < nNode; n++) {
    cyber_shared_ptr<uXML::Node> node = iconListNode->getNode(n);
...
void Device::notifyWait() {
  CyberLink::WaitRandom(DEFAULT_DISCOVERY_WAIT_TIME);
}

const char *Device::getLocationURL(const std::string &host, std::string &buf) {
  return CyberLink::GetHostURL(host, getHTTPPort(), getDescriptionURI(), buf);
}
...
bool Device::announce(const std::string &ifAddr) {
  ...
  for (size_t n = 0; n < childDeviceCnt; n++) {
    cyber_shared_ptr<Device> childDevice = childDeviceList->getDevice(n);
...
bool Device::announce() {
  ...
  size_t nHostAddrs = CyberLink::GetNHostAddresses();
  for (size_t n = 0; n < nHostAddrs; n++) {
    std::string bindAddrBuf;
    const char *bindAddr = CyberLink::GetHostAddress(n, bindAddrBuf);
    if (CyberLink::StringHasData(bindAddr) == false)
      continue;
...
CyberLink::HTTP::StatusCode Device::httpRequestRecieved(CyberLink::HTTPRequest *httpReq) {
  if (Debug::isOn() == true)
    httpReq->print();
...
CyberLink::HTTP::StatusCode Device::httpGetRequestRecieved(HTTPRequest *httpReq) {
  string uri;
  httpReq->getURI(uri);
  if (uri.length() <= 0) {
    return httpReq->returnBadRequest();
  ...
  cyber_shared_ptr<Device> embDev;
  Service *embService;
...
CyberLink::HTTP::StatusCode Device::httpPostRequestRecieved(HTTPRequest *httpReq) {
...
CyberLink::HTTP::StatusCode Device::soapBadActionRecieved(HTTPRequest *soapReq) {
...
CyberLink::HTTP::StatusCode Device::soapActionRecieved(HTTPRequest *soapReq) {
  string uri;
  soapReq->getURI(uri);
...
CyberLink::HTTP::StatusCode Device::invalidActionControlRecieved(ControlRequest *ctlReq) {
  ControlResponse actRes;
  actRes.setFaultResponse(UPnP::INVALID_ACTION);
  return ctlReq->post(&actRes);
}

CyberLink::HTTP::StatusCode Device::deviceControlRequestRecieved(ControlRequest *ctlReq, Service *service) {
...
CyberLink::HTTP::StatusCode Device::deviceActionControlRecieved(ActionRequest *ctlReq, Service *service) {
  if (Debug::isOn() == true)
    ctlReq->print();
...
CyberLink::HTTP::StatusCode Device::deviceQueryControlRecieved(QueryRequest *ctlReq, Service *service) {
  if (Debug::isOn() == true)
    ctlReq->print();
...
CyberLink::HTTP::StatusCode Device::upnpBadSubscriptionRecieved(SubscriptionRequest *subReq, int code) {
  SubscriptionResponse subRes;
  subRes.setErrorResponse(code);
  return subReq->post(&subRes);
}

CyberLink::HTTP::StatusCode Device::deviceEventSubscriptionRecieved(SubscriptionRequest *subReq) {
  if (Debug::isOn() == true)
    subReq->print();
  string uri;
  subReq->getURI(uri);
  ...
  if (subReq->hasCallback() == false && subReq->hasSID() == false) {
    return upnpBadSubscriptionRecieved(subReq, CyberLink::HTTP::PRECONDITION_FAILED);
  }
...
CyberLink::HTTP::StatusCode Device::deviceEventNewSubscriptionRecieved(Service *service, SubscriptionRequest *subReq) {
  string callback;
  ...
  return CyberLink::HTTP::OK_REQUEST;
}

CyberLink::HTTP::StatusCode Device::deviceEventRenewSubscriptionRecieved(Service *service, SubscriptionRequest *subReq) {
  string sidBuf;
...
CyberLink::HTTP::StatusCode Device::deviceEventUnsubscriptionRecieved(Service *service, SubscriptionRequest *subReq) {
  string sidBuf;
  const char *sid = subReq->getSID(sidBuf);
