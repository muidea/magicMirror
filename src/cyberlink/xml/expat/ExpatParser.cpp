...
typedef struct _ExpatData {
  cyber_shared_ptr<Node> rootNode;
  cyber_shared_ptr<Node> currNode;
} ExpatData;
...
static void XMLCALL ExpatElementStart(void *userData, const XML_Char *el, const XML_Char **attr) {
  ...
  cyber_shared_ptr<Node> node(new Node());
  node->setName(el);
...
cyber_shared_ptr<uXML::Node> Parser::parse(const std::string &data, size_t len) {
  ...
  return cyber_shared_ptr<uXML::Node>(expatData.rootNode);
}
