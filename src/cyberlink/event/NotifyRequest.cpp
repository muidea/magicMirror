...
uXML::Node *NotifyRequest::createPropertySetNode(const std::string &varName, const std::string &value) {
  ...
  cyber_shared_ptr<uXML::Node> propNode(new uXML::Node(propXmlNs.c_str()));
  propSetNode->addNode(propNode);

  cyber_shared_ptr<uXML::Node> varNameNode(new uXML::Node(varName));
...
cyber_shared_ptr<uXML::Node> NotifyRequest::getVariableNode() {
  cyber_shared_ptr<uXML::Node> rootNode = getEnvelopeNode();
  if (!rootNode)
    return cyber_shared_ptr<uXML::Node>((uXML::Node *)nullptr);
  if (rootNode->hasNodes() == false)
    return cyber_shared_ptr<uXML::Node>((uXML::Node *)nullptr);
  cyber_shared_ptr<uXML::Node> propNode = rootNode->getNode(0);
...
PropertyList *NotifyRequest::getPropertyList()  {
  propList.clear();
  cyber_shared_ptr<uXML::Node> propSetNode = getEnvelopeNode();
  for (size_t i = 0; i<propSetNode->getNNodes(); i++){
    cyber_shared_ptr<uXML::Node> propNode = propSetNode->getNode(i);
    if (!propNode)
      continue;
